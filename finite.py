...
def eigensystem(n_spin_orbitals, hOp, basis, nPsiMax, groundDiagMode='Lanczos',
    ...
    h = get_hamiltonian_matrix(n_spin_orbitals, hOp, basis,mode='sparse_MPI')
...
def dc_MLFT(nc_i, c, Fcc, nv_i=None, Fvc=None, Gvc=None):
    r"""
    ...
    ----------
    nc_i : int
        Nominal (integer) core-orbital occupation.
    c : float
        Many-body correction to the charge transfer energy.
    nv_i : int
        Nominal (integer) valence-orbital occupation.
    Fcc : list
        Slater integrals {F_{cc}^k}, k \in [0,1,2,3,4]
    Fvc : list
        Slater integrals {F_{vc}^k}, k \in [0,1,2]
    Gvc : list
        Slater integrals {G_{vc}^k}, k \in [0,1,2,3]
    ...
    """
    if not int(nc_i) == nc_i:
        raise ValueError('core-orbital occupation should be an integer')
    if nv_i != None and int(nv_i) != nv_i:
        raise ValueError('valence-orbital occupation should be an integer')

    # Average repulsion energy defines Udd and Upd
    Ucc = Fcc[0] - 14.0/441*(Fcc[2] + Fcc[4])
    if nv_i==None and Fvc==None and Gvc==None:
        return Ucc*n3d_i - c
    if nv_i==6 and Fvc!=None and Gvc!=None:
        Upd = Fvc[0] - (1/15.)*Gvc[1] - (3/70.)*Gvc[3]
        return [Ucc*nc_i+Uvc*n2p_i-c,Uvc*(nc_i+1)-c]
    if Fvc!=None and Gvc!=None:
        Uvc = Fvc[0] - (1/15.)*Gvc[1] - (3/70.)*Gvc[3]
        return [Ucc*nc_i+Uvc*nv_i-c,Uvc*(nc_i)-c]
    else:
...
def get_spherical_2_cubic_matrix(spinpol=False,l=2):
    ...
    l : integer
        Angular momentum number. p: l=1, d: l=2, f: l=3.
    ...
    """
    if l == 1:
        u = np.zeros((3,3),dtype=np.complex64) # complex64 is complex number represented by two single-precision floats
        u[0,0] = 1j/np.sqrt(2)
        u[2,0] = 1j/np.sqrt(2)
        u[0,1] = 1/np.sqrt(2)
        u[2,1] = -1/np.sqrt(2)
        ...
        u = np.zeros((5,5),dtype=np.complex64)
        u[2,0] = 1
        u[[0,-1],1] = 1/np.sqrt(2)
        u[1,2] = -1j/np.sqrt(2)
        u[-2,2] = -1j/np.sqrt(2)
        ...
        u = np.zeros((7, 7), dtype=np.complex64)
        u[0, 1] = np.sqrt(5) / 4
        u[0, 2] = -np.sqrt(5) / 4 * 1j
        u[0, 4] = -np.sqrt(3) / 4
        u[0, 5] = -np.sqrt(3) / 4 * 1j
        u[1, 0] = 1 / np.sqrt(2) * 1j
        u[1, 6] = 1 / np.sqrt(2)
        u[2, 1] = -np.sqrt(3) / 4
        u[2, 2] = -np.sqrt(3) / 4 * 1j
        u[2, 4] = -np.sqrt(5) / 4
        u[2, 5] = np.sqrt(5) / 4 * 1j
        u[3, 3] = 1
        u[4, 1] = np.sqrt(3) / 4
        u[4, 2] = -np.sqrt(3) / 4 * 1j
        u[4, 4] = np.sqrt(5) / 4
        u[4, 5] = np.sqrt(5) / 4 * 1j
        u[5, 0] = -1 / np.sqrt(2) * 1j
        u[5, 6] = 1 / np.sqrt(2)
        u[6, 1] = -np.sqrt(5) / 4
        u[6, 2] = -np.sqrt(5) / 4 * 1j
        u[6, 4] = np.sqrt(3) / 4
        u[6, 5] = -np.sqrt(3) / 4 * 1j

    if spinpol:
        n,m = np.shape(u)
        U = np.zeros((2*n,2*m),dtype=np.complex64)
        U[0:n,0:m] = u
        U[n:,m:] = u
        u = U
    return u
...
def printstate(psi,n_spin_orbitals,printstr):
   print(printstr)
...
def printOp(nBaths,pOp,printstr):
   ...
   print(np.array2string(a, max_line_width=5000,threshold=10000, precision=5, suppress_small=True))
   print("Eigenvalues: ")
   print(np.array_str(np.linalg.eigvalsh(a), max_line_width=368, precision=5, suppress_small=True))
   print('')
   return
...
def getvcSlaterCondonUop(ls,Fcc=[9,0,8,0,6], Fvv=[20,0,8],
                            Fvc=[10,0,8], Gvc=[0,3,0,2]):
    '''
    ...
    FccOp = getUop(l1=lc,l2=lc,l3=lc,l4=lc,R=Fcc)
    # Calculate F_pp^{0,2}
    FvvOp = getUop(l1=lv,l2=lv,l3=lv,l4=lv,R=Fvv)
    # Calculate F_pd^{0,2}
    FvcOp1 = getUop(l1=lv,l2=lc,l3=lc,l4=lv,R=Fvc)
    FvcOp2 = getUop(l1=lc,l2=lv,l3=lv,l4=lc,R=Fvc)
    FvcOp = addOps([FvcOp1,FvcOp2])
    # Calculate G_pd^{1,3}
    GvcOp1 = getUop(l1=lv,l2=lc,l3=lv,l4=lc,R=Gvc)
    GvcOp2 = getUop(l1=lc,l2=lv,l3=lc,l4=lv,R=Gvc)
    GvcOp = addOps([GvcOp1,GvcOp2])
    # Add operators
    uOp = addOps([FccOp,FvvOp,FvcOp,GvcOp])
    return uOp
...
def get_hamiltonian_matrix(n_spin_orbitals, hOp, basis, mode='sparse_serial'):
    """
...
def get_tridiagonal_krylov_vectors(h, psi0, krylovSize, h_local=False,
    ...
    v = np.zeros((2,n), dtype=np.complexfloating)
    # Initialization...
    v[0,:] = psi0
